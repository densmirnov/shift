...
SNAPSHOT_CONFIG="$(pwd)/snapshot.json"
...
SOURCE_DB_NAME="$(grep "database" $SHIFT_CONFIG | cut -f 4 -d '"')"
...
bash shift.sh stop_node -c $SNAPSHOT_CONFIG &> /dev/null
...
bash shift.sh snapshot -s $SNAPSHOT_ROUND -c $SNAPSHOT_CONFIG
